...
class User extends CI_Controller {
    ...
    public function index() {
        if (is_logged_in()) {
            ...
            $response['fasttrack_income'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = "'.$this->session->userdata['user_id'].'" and type = "fasttrack_income"', 'ifnull(sum(amount),0) as fasttrack_income');
            $response['Jackpot_income'] = $this->User_model->get_single_record('tbl_income_wallet', 'user_id = "'.$this->session->userdata['user_id'].'" and type = "Jackpot_income"', 'ifnull(sum(amount),0) as Jackpot_income');
            ...
            $response['prediction_winner'] = $this->User_model->get_records('tbl_prediction_users', 'date(updated_at) = date(now()) && status = 1', '*');
           // pr($response,true);
            $response['popup'] = $this->User_model->getLastSingleRecord('tbl_popup',array(),'media');
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if (!empty($user)) {
                if ($user['disabled'] == 0) {
                    ...
                    $this->getOtp($user['email']);
                    redirect('Dashboard/User/emailVerify');
                } else {
    ...
    public function emailVerify() {
        ...
        $this->load->view('emailVerify', $response);
    }
                ...
                // if(empty($pan_detail)){
                    if(!empty($sponser)){
                        ...
                        if ($res) {
                            ...
                            $this->sendEmail($userData['email'],'Registration',$sms_text);
...
//            pr($response);
    ...
    public function Downline($position = '') {
        if (is_logged_in()) {
            ...
            $response['users'] = $this->User_model->get_records('tbl_downline_business', $where, 'id,downline_id,level,position');
            // echo '<pre>';
    ...
    // public function Downlinetest($position = '') {
    //     if (is_logged_in()) {
    //         $response = array();
    //         $where['upline_id'] = $this->session->userdata['user_id'];
    //         if($position != ''){
    //             $where['position'] = $position;
    //             if($position == 'L')
    //                 $response['header'] = 'Left Downline Participants';
    ...
    //         //$response['users'] = $this->User_model->get_records('tbl_pool', $where, '*');
    //         $response['users'] = $this->getDownline($this->session->userdata['user_id'],$position);
    //         // foreach ($response['users'] as $key => $user) {
    //         //     $response['users'][$key]['user'] = $this->User_model->get_single_record('tbl_users', array('user_id' => $user['downline_id']), 'id,user_id,sponser_id,role,name,email,phone,position,paid_status,upline_id,created_at');
    //         // }
    //         echo '<pre>';
    ...
    // public function getDownline($upline,$position){
    //     $where = ['upline_id' => $upline,'position'=>$position];
    //     $users = $this->User_model->get_records('tbl_pool', $where, '*');
    //     $array[] = $users;
    //     if(!empty($users[0]['user_id'])){
    //         $this->getDownline($users[0]['user_id'],$position);
    //     }else{
    //         echo '<pre>';
    //         print_r($array);
    //         //return $array;
    ...
    public function PoolTree($user_id) {
        if (is_logged_in()) {
            ...
            $this->load->view('poolbinery', $response);
        } else {
            redirect('Dashboard/User/login');
        }
    }
...
//            pr($response,true);
    ...
    public function predictionID(){
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $response['csrfHash'] = $this->security->get_csrf_hash();
                $data = $this->security->xss_clean($this->input->post());
                $userData['prediction'] = $data['prediction'];
                //$todayID = $this->User_model->get_records('tbl_users','date(topup_date) = date(now())','count(id) as record');
                $value = explode(',',$userData['prediction']);
                //if($value[0] <= $todayID[0]['record'] && $value[1] >= $todayID[0]['record']){
                    $incomeArray = [
                ...
                //         'description' => 'Wrong prediction selected '.$userData['prediction'],
                //     ];
                // }
                $result = $this->User_model->add('tbl_prediction_users',$incomeArray);
                // echo '<br>';
                // print_r($incomeArray);
                if($result){
                    $this->session->set_flashdata('message', 'Prediction Process done successfully');
                }else{
                    $this->session->set_flashdata('message', 'Network Error,Please try later');
                }
            }
            $response['ids'] = $this->User_model->get_records('tbl_predictionID',array(),'*');
            $this->load->view('predictionTable',$response);
        }else {
            redirect('Dashboard/User/login');
        }
    }
    ...
    public function rechargeNumber(){
        if((is_logged_in())) {
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $transactionPass = $data['transactionPass'];
                $amount = $data['amount'];
                $phoneNumber = $data['phoneNumber'];
                $CheckTrans = $this->User_model->get_single_record('tbl_users',array('user_id' => $user_id),'master_key');
                if($CheckTrans['master_key'] == $transactionPass){
                    $balance = $this->User_model->get_single_record('tbl_income_wallet',array('user_id' => $user_id),'ifnull(sum(amount),0) as sum');
                    $amountCheck = round($amount/75,2);
                    if($balance['sum'] >= $amountCheck){
                        $Member_id = '121';
                        //$apiPassword = '673645';
                        //$apiPIN = '5574';
                        $request_id = uniqid();
                        $token = '8c39f04deb190c5d51e5c6466c304c85';
                        $url = "http://digitalpayindia.org/API/TransactionAPI?UserID=$Member_id&Token=$token&Account=$phoneNumber&Amount=$amount&SPKey=$operator&APIRequestID=$request_id&Optional1=&Optional2=&Optional3=&Optional4=&Format=2";
                        $curl = curl_init();
                        curl_setopt_array($curl, array(
                          CURLOPT_URL => $url,
                          CURLOPT_RETURNTRANSFER => true,
                        ...
                        // print_r($newArr);
                        // die;
                            if($newArr['STATUS'] == 2 || $newArr['STATUS'] == 1){
                                $userData = [
                                    'user_id' => $user_id,
                                    'amount' => -(round($amount/75,2)),
                                    'type' => 'recharge_amount',
                                    'description' => 'Recharge Done with Number '.$phoneNumber.' with amount Rs.'.$amount,
                                ];
                                ...
                                $UserData2 = [
                                    ...
                                    'rpid' => $newArr['RPID'],
                                    'agent_id' => $newArr['AGENTID'],
                                    'msg' => $newArr['MSG'],
                                    'balance' => $newArr['BAL'],
                                ];
                                $this->User_model->add('mobilerecharge',$UserData2);
                            }
                            if($newArr['STATUS'] == 2){
                                $status = 'SUCCESS';
                            }else{
                        ...
                        if($newArr['STATUS'] == 2){
                            $this->session->set_flashdata('message','Recharge Done Successfully');
                        }else{
                            $this->session->set_flashdata('message',$newArr['MSG']);
                        }
                    }else{
                        $this->session->set_flashdata('message','You have not sufficient balance to recharge');
                    }
                }else{
                    $this->session->set_flashdata('message','Please enter correct Transaction Password');
                }
            }
            $response2['balance'] = $this->User_model->get_single_record('tbl_income_wallet',array('user_id' => $this->session->userdata['user_id']),'ifnull(sum(amount),0) as sum');
            $this->load->view('recharge',$response2);
        }else {
            redirect('Dashboard/User/login');
        }
    }
    ...
    {   
        ...
        //     if ($this->input->server('REQUEST_METHOD') == 'GET') {
                ...
                $this->sendEmail($email,'Verification',$message);
