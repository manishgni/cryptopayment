        ...
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if($userDetail['paid_status'] >= 0){
                    ...
                    $bank = $this->User_model->get_single_record('tbl_bank_details',['user_id' => $this->session->userdata['user_id']],'nominee,nominee_relation');
                    //if(empty($get['city'])){
                    ...
                    // }
                     // if(empty($get['country'])){
                        ...
                        $Userdata5['country_code'] = $data['phonecode'];
                    ...
                    // }
                    // if(empty($bank['nominee'])){
                        $updres = $this->User_model->update('tbl_bank_details', array('user_id' => $this->session->userdata['user_id']), ['nominee' => $data['nominee']]);
                    // }
                    // if(empty($bank['nominee_relation'])){
                        $updres = $this->User_model->update('tbl_bank_details', array('user_id' => $this->session->userdata['user_id']), ['nominee_relation' => $data['nominee_relation']]);
                    //}
                    if (!empty($updres)) {
        ...
        if (is_logged_in()) {
            if ($this->input->is_ajax_request()) {
                if ($this->input->server('REQUEST_METHOD') == 'GET') {
                    ...
                    if(!empty($_SESSION['verification_otp']) && $_SESSION['verification_otp'] == $otp){
                        $this->User_model->update('tbl_users', ['user_id' => $this->session->userdata['user_id']] ,['email_verify_status' => 1]);
                        $response['success'] = 1;
                        $response['message'] = 'Email Verified Successfully!';
                        $response['url'] = base_url('Dashboard/Profile');
                    }else{
                        $response['success'] = 0;
        ...
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                // } else {
                    ...
                    if ($updres == true) {
                        $response['message'] = 'USDT Address Updated Successfully';
                        $response['success'] = 1;
                        $this->session->set_flashdata('message','USDT Address Updated Successfully');
                        redirect('Dashboard/Profile/usdtaddress');
                    } else {
                        $response['message'] = 'There is an error while Updating USDT Address Please Try Again';
                        redirect('Dashboard/Profile/usdtaddress');
                    }
                // }
            }
            $response['user_bank'] = $this->User_model->get_single_record('tbl_bank_details', array('user_id' => $this->session->userdata['user_id']), '*');
              $this->load->view('crypto_address',$response);
        ...
        if (is_logged_in()) {
            if ($this->input->is_ajax_request()) {
                if ($this->input->server('REQUEST_METHOD') == 'GET') {
                    ...
                    // $msg = "<div style=\'background-color: #000;padding:50px 0px;\'><div style=\'text-align:center;\'><div><img src=\'https://artisticuniversal.com/uploads/logo.png\' alt=\'logo\' style=\'max-width: 200px; background-color: #fff;\'></div></div><div><h3 style=\'font-size: 18px;color: #fff;text-align: center;\'>'.$data['info']['title']</h3></div><div style=\'text-align:center;margin: 0px auto;\'><div style=\'color: #fff;\'><h3>Dear '.$data['info']['name'].',<br><br>You OTP is 264489 One Time Password, this otp expire in 2 mintues!<p style=\'color:#fff;\'>Please never share this OTP (one time password) With Anyone!</h3></p></div></div></div>"
        ...
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                // } else {
                    $updres = $this->User_model->update('tbl_bank_details', array('user_id' => $this->session->userdata['user_id']), $user_data);
                    if ($updres == true) {
                        $response['message'] = 'Tron Address Updated Successfully';
                        $response['success'] = 1;
                        $this->session->set_flashdata('message','Tron Address Updated Successfully');
                        redirect('Dashboard/Profile/accountDetails');
                    } else {
                        $response['message'] = 'There is an error while Updating Tron Address Please Try Again';
                        redirect('Dashboard/Profile/accountDetails');
                    }
                // }
            }
