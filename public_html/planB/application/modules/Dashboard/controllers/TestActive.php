...
class TestActive extends CI_Controller {
    ...
    public function activationTrail($value='')
    {
        if (is_logged_in()) {
            //die('here');
            $response['user'] = $this->User_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['user_id']), '*');
                        ...
                        if (!empty($user)) {
                            if ($wallet['wallet_balance'] >= $package['price']) {
                                if ($user['paid_status'] == 0) {
                                        ...
                                        //}
                                        $this->session->set_flashdata('message', '<h3 class="text-success">Account Activated Successfully </h3>');
                                        redirect('Dashboard/Activation/activationTrail');
                                } else {
                                    $this->session->set_flashdata('message', '<h3 class="text-danger">This Account Already Acitvated </h3>');
                                }
                            } else {
            ...
            $response['wallet'] = $this->User_model->get_single_record('tbl_wallet', array('user_id' => $this->session->userdata['user_id']), 'ifnull(sum(amount),0) as wallet_balance');
            $response['packages'] = $this->User_model->get_records('tbl_package',[], '*');
            $this->load->view('activationTrail', $response);
        } else {
            redirect('Dashboard/User/login');
        }
    }
    ...
    //     //$userArr = ['AU94745','AU19102','AU94375'];
    //     $userArr = ['AU57671','AU37311','AU75020'];
    ...
    //                 $table = 'tbl_pool'.$i;
    //             }
    //             echo 'user '.$user. ' | table '.$table.'<br>';
    //             $this->globlePoolEntry($user,$table,1);
    //         } 
    //     }
    // }
    ...
    private function poolIncome2($table,$user_id,$linkedID,$org,$pool_id){
        ...
        if(!empty($upline['upline_id'])){
            ...
            $userinfo = $this->User_model->get_single_record('tbl_users',['user_id' => $upline['upline_id']],'*');
            if($checkTeam['team'] == 3):
                $creditIncome = [
                    'user_id' => $upline['upline_id'],
                ...
                $this->User_model->add('tbl_income_wallet',$creditIncome);
                //if($userinfo['package_amount'] < $poolData['package']){
                    $debitIncome = [
                        'user_id' => $upline['upline_id'],
                ...
                if($poolData['org'] == 4 && $poolData['table'] != ' ' && $poolData['table'] != $table){
                    if(!empty($poolData['package'])){
                        if($userinfo['package_amount'] < $poolData['package']){
                            $debitIncome = [
                                'user_id' => $upline['upline_id'],
                                'amount' => -$poolData['package'],
                                'type' => 'upgrade_deduction',
                                'description' => 'Pool Upgrade Deduction For level '.$poolData['org'],
                            ];
                            $this->User_model->add('tbl_income_wallet',$debitIncome);
                            $topupData = array(
                                'package_id' => $poolData['package_id'],
                                'package_amount' => $poolData['package'],
                                'upgrade_id' => $poolData['package_id'],
                                'upgrade_amount' => $poolData['package'],
                            );
                            $this->User_model->update('tbl_users',['user_id' => $upline['upline_id']],$topupData);
                    ...
                    $creditUniversal = [
                        ...
                        'description' => 'Universal Pool Income at level '.$poolData['org'],
                        ...
                        if (!empty($user)) {
                            if ($wallet['wallet_balance'] >= $package['price']) {
                                if($nextPackage == $package['id']) {
                                        ...
                                        // $josnData = array(
                                        //     count($oldTopupdata) => ['upgrade_id' => $package['id'],'upgrade_amount' => $package['price']],
                                        // );
                                        ...
                                        if($sponser['paid_status'] == 1){
                                            $DirectIncome = array(
                                                ...
                                                'amount' => $package['price']*0.15,
                                                'type' => 'level_income',
                                                'description' => 'Level Income from upgrade of Member ' . $user_id . ' At level 1',
                                            );
                                            $this->User_model->add('tbl_income_wallet', $DirectIncome);
                                        }
                                        $this->level_income($sponser['sponser_id'], $user['user_id'], $package['price']);
    ...
    public function UpgradeAccountTrail() {
        if (is_logged_in()) {
            //$response['packages'] = $this->config->item('packages');
