...
//            $response['requests'] = $this->User_model->get_records('tbl_payment_request', array('user_id' => $this->session->userdata['user_id']), '*');
    ...
    public function Invoice() {
        if (is_logged_in()) {
            $response['userInfo'] = $this->User_model->get_single_record('tbl_users',['user_id' => $this->session->userdata['user_id']],'*');
            $response['siteInfo'] = $this->User_model->get_single_record('tbl_site_content',[''],'address,phone,email');
    ...
    protected function ticket_id(){
        $ticket_id = rand('10000','99999');
        $getOrderID = $this->User_model->get_single_record('tbl_support_message',['ticket_id' => $ticket_id],'ticket_id');
        if(!empty($getOrderID)) {
