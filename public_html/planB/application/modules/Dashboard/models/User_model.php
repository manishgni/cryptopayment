...
class User_model extends CI_Model {
    ...
    public function get_records_asc($table, $where, $select) {
        $this->db->select($select);
        $this->db->where($where);
...
//        echo $this->db->last_query();
    ...
    public function count_level_directs($user_id) {
        $this->db->select('count(id) as level_count,level');
        $this->db->from('tbl_sponser_count');
        $this->db->where(array('user_id' => $user_id ,'level >'=> 0));
        $this->db->group_by('level');
        $this->db->having('level <=',10);
    ...
    public function count_level_directss($user_id) {
        ...
        $this->db->from('tbl_sponser_count');
        $this->db->where(array('user_id' => $user_id ,'level >'=> 0, 'paid_status >' => 0));
        $this->db->group_by('level');
    ...
    public function get_pool_team($pool,$team,$user_id) {
        $this->db->select('team');
        $this->db->from($pool);
        $this->db->where(array('user_id' => $user_id ,'team >='=> $team));
        $res = $query->row_array();
        return $res;
    }


      public function get_like($select,$where,$like) {
        $this->db->select($select);
         $this->db->where($where);
